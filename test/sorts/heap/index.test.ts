import heap from '../../../src/lib/sorts/heap';
...
describe('heap sort', () => {
  ...
  test('cases[0]: ' + cases[0], async () => {
    const obj = { arr: [5] };
    const { arr: arrOfNums } = await heap(obj);
    expect(arrOfNums).toEqual([5]);

    const obj1 = { arr: [] };
    const { arr: arrOfNums1 } = await heap(obj1);
  ...
  test('cases[3]: ' + cases[3], async () => {
    ...
    const { arr: arrOfObjs } = await heap(obj);
  ...
  // 		},
  // 	}
  // 	const { arr: arrOfNums, animate } = await heap(obj)
  // 	expect(arrOfNums).toEqual(sorted().arr.numbers)
