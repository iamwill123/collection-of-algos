...
describe('Insertion sort', () => {
	...
	test('cases[0]: ' + cases[0], async () => {
		const obj = { arr: [5] }
		const { arr: arrOfNums } = await insertion(obj)
		expect(arrOfNums).toEqual([5])

		const obj1 = { arr: [] }
		const { arr: arrOfNums1 } = await insertion(obj1)
	...
	test('cases[3]: ' + cases[3], async () => {
		...
		const { arr: arrOfObjs } = await insertion(obj)
	...
	test('cases[6]: ' + cases[6], async () => {
		...
		const { arr: arrOfNums, animate } = await insertion(obj)
		expect(arrOfNums).toEqual(sorted().arr.numbers)
